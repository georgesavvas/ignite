...
interface ProcessProps {
  ...
  forceKill: (processId: string) => void;
...
const Process = (props: ProcessProps) => {
  ...
  const getStatusIcon = () => {
    switch (state) {
      case "finished":
        return <CheckIcon className={styles.statusIcon} style={getIconStyle("finished")} />;
      case "paused":
        return <PauseIcon className={styles.statusIcon} style={getIconStyle("paused")} />;
      case "waiting":
        return (
          <WatchLaterOutlinedIcon className={styles.statusIcon} style={getIconStyle("waiting")} />
        );
      case "error":
        return <ErrorIcon className={styles.statusIcon} style={getIconStyle("error")} />;
      default:
        return (
          <DirectionsRunOutlinedIcon
            className={styles.statusIcon}
            style={getIconStyle("running")}
          />
        );
    }
  };
  ...
  const getActionButton = () => {
    switch (state) {
      case "paused":
        return (
          <PlayArrowIcon
            className={`${styles.button} ${styles.unpauseButton}`}
  ...
  const getIconStyle = (state: keyof typeof COLOURS) => {
    ...
    switch (state) {
      ...
      default:
        return {
          ...
          transition:
            progress === 0 ? "width 0.1s, background-color 1s" : "width 0.5s, background-color 1s",
        };
    }
  };
    ...
    <div className={styles.container} style={props.style}>
      <div className={styles.topBar}>
        ...
        <div
          ...
          style={{ ...getProgressBarStyle(), backgroundColor: COLOURS[state] }}
        />
      </div>
      <div className={styles.details}>
        ...
        <div className={styles.detailsText}>
          ...
          <Typography noWrap style={{ direction: "rtl", textAlign: "left" }}>
            {props.process.entity.path}
          </Typography>
        ...
        <ClearIcon
          className={`${styles.button} ${
            ["finished", "error"].includes(state) ? styles.pauseButton : styles.killButton
          }`}
          onClick={handleClear}
        />
      </div>
